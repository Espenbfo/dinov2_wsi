...
CHECKPOINT_PATH = Path(
    "weights/teacher_checkpoint-8.pth"
...
FILENAME = f"weights{'-'.join(map(lambda x: str(x), SIZES))}.pt"
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        total_loss = 0
        print("TRAIN")
        for index, (batch, label) in (pbar := tqdm(enumerate(dataloader_train), total=len(dataloader_train), dynamic_ncols=True, colour="cyan")):
        ...
        val_loss = 0
        with torch.no_grad():
            for index, (batch, label) in (pbar := tqdm(enumerate(dataloader_val), total=len(dataloader_val), dynamic_ncols=True, colour="green")):
                batch = (x.to(DEVICE) for x in batch)
                label = label.to(DEVICE)
                label -= 1
                result = model(*batch)
