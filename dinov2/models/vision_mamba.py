...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=8, in_chans=3, embed_dim=768, norm_layer=None,
                 flatten=True):
        super().__init__()
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert H % self.patch_size[0] == 0, f"Input image height {H} is not a multiple of patch height {patch_H}"
        assert W % self.patch_size[1] == 0, f"Input image width {W} is not a multiple of patch width: {patch_W}"
        x = self.proj(x)
        if self.flatten:
...
class VisionMamba(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h, token_position=0):
        ...
        return torch.cat((patch_pos_embed[:,:token_position,:], class_pos_embed.unsqueeze(0), patch_pos_embed[:,token_position:]), dim=1).to(previous_dtype)

    @torch.jit.ignore()
    def load_pretrained(self, checkpoint_path, prefix=""):
        _load_weights(self, checkpoint_path, prefix)

    def forward_features(self, x, inference_params=None, masks=None, if_random_cls_token_position=False,
                         if_random_token_rank=False):
        # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
        # with slight modifications to add the dist_token
        if isinstance(x, list):
            output = []
            for x_s, mask in zip(x, masks):
                output.append(self.forward_features(x_s, inference_params, masks=mask))
            return output
        _, _, w, h = x.shape
        ...
        if self.if_cls_token:
            if self.use_double_cls_token:
                ...
                M = x.shape[1]
        x = x + self.interpolate_pos_encoding(x, w, h, token_position)
        ...
        return {
            "x_norm_clstoken": hidden_states[:, token_position, :],
            "x_norm_regtokens": hidden_states[:, 1: 1],
            "x_norm_patchtokens": torch.cat((hidden_states[:,:token_position,:], hidden_states[:,token_position+1:,:]), dim = 1),
            "x_prenorm": residual,
            "masks": masks}
    ...
    def forward(self, x, is_training=False, inference_params=None, if_random_cls_token_position=False,
                if_random_token_rank=False, *args, **kwargs):
        x = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position,
                                  if_random_token_rank=if_random_token_rank, *args, **kwargs)
        if is_training:
            return x
        x = self.head(x)
...
def vim_tiny_orig(patch_size=16, num_register_tokens=0, **kwargs):
    model = VisionMamba(
        patch_size=patch_size, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, num_classs=0,**kwargs)
    model.default_cfg = _cfg()
    return model

def vim_base_orig(patch_size=16, num_register_tokens=0, **kwargs):
    model = VisionMamba(
        patch_size=patch_size, embed_dim=512, depth=36, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, num_classs=0,**kwargs)
    model.default_cfg = _cfg()
    return model