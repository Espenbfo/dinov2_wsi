...
class VSSM(nn.Module):
    def __init__(
        ...
        self.embed_dim=dims[-1]*2
        for i in range(self.num_layers - 2):
            self.embed_dim += dims[i+1]
        self.interpolate_offset = interpolate_offset
        self.interpolate_antialias = interpolate_antialias

        self.dims = dims
        self.mask_token = nn.Parameter(torch.zeros(1, dims[0]))
        self.pos_embed = nn.Parameter(torch.zeros(1, (img_size//patch_size)**2 + 1, self.dims[0]))
        trunc_normal_(self.pos_embed, std=0.02)
        self.pos_drop = nn.Dropout(p=0.1)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        self.cls_token = nn.Parameter(torch.zeros(1, 1, self.dims[0]))
        ...
        self.classifier = nn.Identity()#nn.Sequential(OrderedDict(
    ...
    def forward(self, x: torch.Tensor, is_training=False, inference_params=None, *args, **kwargs):
        x = self.forward_features(x, inference_params, *args, **kwargs)
        if is_training:
            return x
        x = self.classifier(x["x_norm_clstoken"])
        return x
    ...
    def forward_features_list(self, x_list, masks_list):
        ...
        for x, masks in zip(all_x, masks_list):
            ...
            if masks is not None:
                tempmasks = torch.repeat_interleave(masks.view((x.shape[0],x.shape[1]//(2**(len(self.dims)-1)),x.shape[2]//(2**(len(self.dims)-1)))), (2**(len(self.dims)-1)),dim=1)
                tempmasks = torch.repeat_interleave(tempmasks, (2**(len(self.dims)-1)), dim=2)
                x = torch.where(tempmasks.unsqueeze(-1), self.mask_token.to(x.dtype).unsqueeze(0), x)
            x = x + self.interpolate_pos_encoding(x, w, h).view(x.shape[1:])
            dim_output = []
            ...
            for i in range(len(self.layers) - 2):
                dim_output[i] = nn.functional.max_pool2d(dim_output[i].permute((0, 3, 1, 2)),
                                                         2 ** (len(self.layers) - 2 - i)).permute((0, 2, 3, 1))
            patchtokens = torch.cat(dim_output, dim=-1).view((x.shape[0], -1, self.embed_dim))
            ...
            output.append({
                ...
                "masks": masks,
                "hierarchic_tokens": dim_output
            })
        return output

    def forward_features(self, x, inference_params=None, masks=None, *args, **kwargs):
        ...
        x = self.patch_embed(x)
        if masks is not None:
            tempmasks = torch.repeat_interleave(masks.view(
                (x.shape[0], x.shape[1] // (2 ** (len(self.dims) - 1)), x.shape[2] // (2 ** (len(self.dims) - 1)))),
                                                (2 ** (len(self.dims) - 1)), dim=1)
            tempmasks = torch.repeat_interleave(tempmasks, (2 ** (len(self.dims) - 1)), dim=2)
            x = torch.where(tempmasks.unsqueeze(-1), self.mask_token.to(x.dtype).unsqueeze(0), x)
            x = torch.where(masks.unsqueeze(-1).view(x.shape[1:]), self.mask_token.to(x.dtype).unsqueeze(0), x)
        x = x + self.interpolate_pos_encoding(x, w, h).view(x.shape[1:])
        dim_output = []
        for layer in self.layers:
            x = layer(x)
        ...
        for i in range(len(self.layers)-2):
            dim_output[i] = nn.functional.max_pool2d(dim_output[i].permute((0,3,1,2)), 2**(len(self.layers)-2-i)).permute((0,2,3,1))
        patchtokens = torch.cat(dim_output, dim=-1).view((x.shape[0],-1, self.embed_dim))
...
def vmamba_T(patch_size=16, num_register_tokens=0, **kwargs):
    model = VSSM(
...
def vmamba_S(patch_size=16, num_register_tokens=0, **kwargs):
    model = VSSM(
            patch_size=patch_size//4,
            dims=[96, 192, 384],
            depths=[2,2,9],
